            ...
            document.getElementById("history").innerHTML = document.getElementById("history").innerHTML+searchInput+"<br/>";    
                ...
                if(searchInput.indexOf(":")>-1){
                    ...
                    if(-1 < facetable.indexOf(filterName)){
                        ...
                        if(filtersCount==0){
                            fInput = formatNewFilterInput(fName, filterValue);
                            //check if there is already a keyword filter  
                            if(qHistory.length==0){
                                URL = URL+'filters={"$and":['+fInput+']}';
                            }else{
                                URL = URL+'&filters={"$and":['+fInput+']}';
                            }
                            
                        }else if(filtersHistory[fName]==0){
                            URL = URL.slice(0, URL.indexOf('filters={"$and":[')+17)+fInput+','+URL.slice(URL.indexOf('filters={"$and":[')+17); 
                            
                        }else if(filtersHistory[fName]>0){
                            URL = URL.split('"'+fName+'":{"$eq"').join('"'+fName+'":{"$in"');
                            console.log("splitJoin"+URL);
                            URL = URL.slice(0, (URL.indexOf(fName+':{"$in":[')+fName.length+9))+filterValue+','+URL.slice(URL.indexOf(fName+':{"$in":[')+fName.length+9);
                        }
                        filtersHistory[fName]+=1;
                        filtersCount += 1;
                        console.log(URL);
                        //http://www.factual.com/data/t/places#filters={"$and":[{"post_town":{"$eq":"90024"}},{"post_town":{"$eq":"90024"}}]}
                        //http://www.factual.com/data/t/places#filters={"$and":[{"post_town":{"$eq":"90024"}},{"post_town":{"$eq":"90024"}}]}&q="cheese"
                        //http://www.factual.com/data/t/places#filters={"$and":[{"post_town":{"$eq":"90024"}},{"post_town":{"$eq":"90024"}},{"post_town":{"$eq":"90024"}}]}&q="cheese"
                        window.open(URL);
                    }else{
